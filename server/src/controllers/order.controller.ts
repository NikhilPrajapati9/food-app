...
export const createCheckoutSession = asyncHandler(async (req: Request, res: Response) => {
    ...
    const order = new Order({
        ...
        items: checkoutSessionRequest.cartItems,
        deliveryDetails: checkoutSessionRequest.deliveryDetails,
        // totalAmount: checkoutSessionRequest.cartItems.reduce((total, item) => total + item.price * item.quantity, 0),
        status: "pending"
    })
    ...
    await order.save();
    return res.status(200).json(new ApiResponse(200, { session }, "Checkout session created successfully"));
})
